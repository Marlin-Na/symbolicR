...
(I intended to provide an operator `%:%` (or with another name) to assign symbolic
expressions, by doing so, the variables on the right side (may be identified with `all.vars`)
will automatically substituted as `sym("variable_name")`, and all binary operators and
math functions will be substituted as the corresponding sympy functions. While expressions
wrapped with `.()` will not be substituted and evaluated in R (as inspired by `bquote`).
For example, `z %:% a * sin(x) + .(y)` may be equivalent to `z <- sym("a") * sin(sym("x")) + y`.)
